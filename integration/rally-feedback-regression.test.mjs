...
for(let i=0;i<600;i++)stats=dust.update(1/60,sliding,car,540);assert(stats.active>0&&stats.active<=256);assert.equal(scene.children.length,1);dust.quality('balanced');for(let i=0;i<180;i++)stats=dust.update(1/60,sliding,car,540);assert(stats.active<=128);assert.equal(stats.capacity,128);
