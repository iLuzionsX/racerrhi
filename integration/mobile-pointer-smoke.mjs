...
const readUi = () => page.evaluate(async () => {
  const { input } = await import('./ui.js?v=6');
  return {
    ...input,
...
await waitUi(async () => (await import('./ui.js?v=6')).input.held === true, 'wheel A did not acquire analog ownership');
...
await waitUi(async () => Math.abs((await import('./ui.js?v=6')).input.steer) > 0.10, 'wheel A did not create steering');
await gasA.end();
...
await wheelA.end();
await waitUi(async () => (await import('./ui.js?v=6')).input.held === false, 'wheel stayed held after gas-first cleanup');
...
await waitUi(async () => (await import('./ui.js?v=6')).input.held === true, 'wheel B did not acquire analog ownership');
const wheelPointerB = await activePointerId('wheel');
await wheelB.move(cx, wheelEndY);
await wheelB.end();
await waitUi(async () => (await import('./ui.js?v=6')).input.held === false, 'wheel-first release stayed held');
...
await waitUi(async () => (await import('./ui.js?v=6')).input.held === true, 'wheel C did not acquire analog ownership');
const wheelPointerC = await activePointerId('wheel');
const otherC = await page.touchscreen.touchStart(ox, oy);
await otherC.end();
await waitUi(async () => (await import('./ui.js?v=6')).input.held === true, 'unrelated finger release cleared steering ownership');
...
await wheelC.end();
await waitUi(async () => (await import('./ui.js?v=6')).input.held === false, 'wheel C cleanup stayed held');
...
await waitUi(async () => (await import('./ui.js?v=6')).input.held === true, 'first immediate grab failed');
const firstGrabPointerId = await activePointerId('wheel');
await wheelD1.end();
await waitUi(async () => {
  const { input } = await import('./ui.js?v=6');
  return input.held === false && input.steer === 0;
}, 'first immediate release did not center steering');
const wheelD2 = await page.touchscreen.touchStart(wheelStartX, cy);
await waitUi(async () => (await import('./ui.js?v=6')).input.held === true, 'second immediate grab failed');
const secondGrabPointerId = await activePointerId('wheel');
await wheelD2.move(cx, wheelEndY);
await waitUi(async () => Math.abs((await import('./ui.js?v=6')).input.steer) > 0.10, 'second immediate grab did not steer');
await wheelD2.end();
await waitUi(async () => (await import('./ui.js?v=6')).input.held === false, 'second immediate release stayed held');
...
await waitUi(async () => (await import('./ui.js?v=6')).input.held === true, 'keyboard handoff touch did not acquire analog ownership');
...
await wheelE.end();
await waitUi(async () => (await import('./ui.js?v=6')).input.held === false, 'touch release with key held left analog owner active');
await page.waitForFunction(() => {
  const d = globalThis.__racerrhiDiagnostics?.lastPhysicsInput;
  return d &&
...
await waitUi(async () => (await import('./ui.js?v=6')).input.held === true, 'focus-loss setup did not hold wheel');
...
if (visibilityLossObserved) {
  ...
  await page.bringToFront();
  await wheelF.end();
  await waitUi(async () => (await import('./ui.js?v=6')).input.held === false, 'focus-loss fallback cleanup stayed held');
}
await cover.close();
