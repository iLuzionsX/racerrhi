...
renderer.setPixelRatio(Math.min(devicePixelRatio,mobile?1.4:1.8));renderer.setSize(innerWidth,innerHeight);renderer.outputColorSpace=T.SRGBColorSpace;renderer.shadowMap.enabled=true;renderer.shadowMap.type=T.PCFSoftShadowMap;renderer.shadowMap.autoUpdate=true;renderer.toneMapping=T.ACESFilmicToneMapping;renderer.toneMappingExposure=1.02;
const scene=new T.Scene();scene.fog=new T.FogExp2('#adb9bc',.00055);const camera=new T.PerspectiveCamera(48,innerWidth/innerHeight,.15,6500);
...
scene.add(new T.HemisphereLight('#dceeff','#59684a',.66));const sunlight=new T.DirectionalLight('#ffdda1',3.55);sunlight.castShadow=true;sunlight.shadow.mapSize.set(mobile?1024:2048,mobile?1024:2048);Object.assign(sunlight.shadow.camera,{left:-55,right:55,top:55,bottom:-55,near:1,far:260});sunlight.shadow.bias=-.00012;sunlight.shadow.normalBias=.022;sunlight.shadow.radius=2;scene.add(sunlight,sunlight.target);
...
let runoffBuilt=false;const buildRunoff=()=>{if(runoffBuilt)return;runoffBuilt=true;ribbon(0,47,dirtMat,-.14);ribbon(0,33,sandMat,-.09);};ribbon(0,15,roadMat);ribbon(-7.7,.9,white,.045,true);ribbon(7.7,.9,white,.045,true);ribbon(-7.19,.13,white,.06);ribbon(7.19,.13,white,.06);
...
function rail(offset,h){const ps=[],ix=[];for(let i=0;i<=N;i++){const a=samples[i%N],p=a.p.clone().addScaledVector(a.n,offset);ps.push(p.x,p.y+h-.13,p.z,p.x,p.y+h+.13,p.z);if(i<N){const j=i*2;ix.push(j,j+1,j+2,j+1,j+3,j+2);}}const g=new T.BufferGeometry();g.setAttribute('position',new T.Float32BufferAttribute(ps,3));g.setIndex(ix);g.computeVertexNormals();const m=metal.clone();m.side=T.DoubleSide;mesh(g,m);}
for(const side of [-1,1]){for(const h of [.65,1.08])rail(side*16,h);const count=Math.floor(length/5),inst=new T.InstancedMesh(new T.BoxGeometry(.15,1.3,.15),metal,count);const dummy=new T.Object3D();for(let i=0;i<count;i++){const a=at(i/count);dummy.position.copy(a.p).addScaledVector(a.n,side*16);dummy.position.y+=.6;dummy.rotation.y=Math.atan2(a.d.x,a.d.z);dummy.updateMatrix();inst.setMatrixAt(i,dummy.matrix);}scene.add(inst);}
...
function ground(x,z){let r=nearest(x,z,10);if(r.distance<60)r=nearest(x,z);const base=r.p.y-.65;const dist=r.distance;let y=base+(Math.sin(x*.019)*Math.cos(z*.023)*4+Math.sin(z*.047)*1.2)*clamp((dist-23)/60,0,1);y+=Math.pow(Math.max(0,(x-310)/400),1.6)*110;y-=Math.max(0,(-x-275))*.37;return Math.max(-12,y);}
...
for(let j=0;j<8;j++){const g=new T.PlaneGeometry(650,600,48,48);g.rotateX(-Math.PI/2);const ps=g.attributes.position;for(let i=0;i<ps.count;i++){const x=ps.getX(i),z=ps.getZ(i);ps.setY(i,Math.max(0,1-Math.hypot(x/340,z/330))*230+Math.sin(x*.032)*Math.cos(z*.037)*13);}g.computeVertexNormals();mesh(g,rockMat,650+j%2*360,0,-1200+j*350);}
// Instanced vegetation keeps the circuit lightweight on phones.
const treePositions=[];for(let i=0;i<1700&&treePositions.length<470;i++){const x=-320+rand()*1050,z=-690+rand()*1400,r=nearest(x,z,9);if(r.distance<24||x< -280||ground(x,z)<2)continue;treePositions.push({x,z,y:ground(x,z),s:4+rand()*6});}
const dummy=new T.Object3D();
let reloadSurfaceQuality=async()=>{},runoffQualityReady=false;
const visualReady=Promise.all([surfaces(scene,renderer,{asphalt:roadMat,grass:terrainMat,rock:rockMat,sand:sandMat,dirt:dirtMat},sky).then(reload=>{reloadSurfaceQuality=reload;}),foliage(scene,treePositions)]);
...
furniture(scene,at,length);const trackDetailQuality=trackDetail(scene,at,length);
...
function quality(){reflections?.quality(config.quality);trackDetailQuality(config.quality);if(runoffQualityReady)reloadSurfaceQuality(config.quality);renderer.setPixelRatio(Math.min(devicePixelRatio,config.quality==='high'?2:mobile?1.25:1.5));sunlight.shadow.mapSize.setScalar(config.quality==='high'?(mobile?2048:4096):1024);sunlight.shadow.radius=config.quality==='high'?1.5:2.5;if(sunlight.shadow.map){sunlight.shadow.map.dispose();sunlight.shadow.map=null;}}
