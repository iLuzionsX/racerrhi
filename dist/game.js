...
function quality(){reflections?.quality(config.quality);if(runoffQualityReady&&config.quality==='balanced')reloadSurfaceQuality('balanced');renderer.setPixelRatio(Math.min(devicePixelRatio,config.quality==='high'?2:mobile?1.25:1.5));sunlight.shadow.mapSize.setScalar(config.quality==='high'?(mobile?2048:4096):1024);if(sunlight.shadow.map){sunlight.shadow.map.dispose();sunlight.shadow.map=null;}}
