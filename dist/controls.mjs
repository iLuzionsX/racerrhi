...
export const defaults=()=>({version:2,show:true,sensitivity:1,keyboardResponse:1.25,keyboardStrength:1,wheelMode:'drag',wheelSize:210,pedalSize:78,wheelX:.02,wheelY:.93,pedalX:.98,pedalY:.93,quality:'balanced',sound:false});
export function sanitize(raw={}){raw=raw||{};const d=defaults();for(const k of ['wheelSize','pedalSize','wheelX','wheelY','pedalX','pedalY','sensitivity','keyboardResponse','keyboardStrength'])if(Number.isFinite(raw[k]))d[k]=raw[k];d.wheelSize=clamp(d.wheelSize,140,300);d.pedalSize=clamp(d.pedalSize,65,115);for(const k of ['wheelX','wheelY','pedalX','pedalY'])d[k]=clamp(d[k],0,1);d.sensitivity=clamp(d.sensitivity,.6,1.5);d.keyboardResponse=clamp(d.keyboardResponse,.7,1.8);d.keyboardStrength=clamp(d.keyboardStrength,.75,1.25);d.wheelMode=raw.wheelMode==='rotate'?'rotate':'drag';d.quality=raw.quality==='high'?'high':'balanced';d.show=raw.show!==false;d.sound=raw.sound===true;return d;}
